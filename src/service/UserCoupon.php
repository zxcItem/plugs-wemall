...
use plugin\wemall\model\AccountUserCoupon;
...
abstract class UserCoupon
{
     ...
     * @return AccountUserCoupon
     * @throws Exception
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\DbException
     * @throws \think\db\exception\ModelNotFoundException
     */
    public static function create($unid, int $coid): AccountUserCoupon
    {
        [$rela, $unid] = AccountRelation::withRelation($unid);
        // 检查卡券
        $where = ['id' => $coid, 'status' => 1, 'deleted' => 0];
        $coupon = ShopConfigCoupon::mk()->where($where)->findOrEmpty();
        ...
        if (($limitTimes = $coupon->getAttr('limit_times')) > 0) {
            ...
            if (AccountUserCoupon::mk()->where($map)->count() > $limitTimes) {
        ...
        while (($model = AccountUserCoupon::mk()->where(['code' => $code])->findOrEmpty())->isExists());
        // 保存及返回模型
    ...
    /**
     * 重置卡券统计
     * @param int|ShopConfigCoupon $coid
     * @return boolean
     * @throws Exception
     */
    public static function recount($coid): bool
    {
        ...
        $total = AccountUserCoupon::mk()->field($field)->where($where)->findOrEmpty()->toArray();
     ...
     * @return AccountUserCoupon
     */
    public static function resume(string $code): AccountUserCoupon
    {
        $coupon = AccountUserCoupon::mk()->where(['code' => $code, 'status' => 2])->findOrEmpty();
     ...
     * @throws Exception
     */
    public static function confirm(string $code): AccountUserCoupon
    {
        $map = ['code' => $code, 'status' => 1];
        if (($coupon = AccountUserCoupon::mk()->where($map)->findOrEmpty())->isExists()) {
    ...
    /**
     * 获取优惠券模型
     * @param int|ShopConfigCoupon $model
     * @return ShopConfigCoupon
     * @throws Exception
     */
    public static function withModel($model): ShopConfigCoupon
    {
        if (is_numeric($model)) {
            $model = ShopConfigCoupon::mk()->where(['id' => $model])->findOrEmpty();
        }
        if ($model instanceof ShopConfigCoupon) {
