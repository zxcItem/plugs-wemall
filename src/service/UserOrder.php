...
use plugin\wemall\model\ShopOrderSend;
...
class UserOrder
{
    ...
    public static function upgrade(string $orderNo): ?array
    {
        ...
        // 会员用户数据
        $map = ['unid' => $order['unid']];
        $user = ShopUserRelation::mk()->where($map)->findOrEmpty();
    ...
    private static function _vipEntry(int $unid): int
    {
        ...
        // 更新用户支付信息
        ShopUserRelation::mk()->where(['unid' => $unid])->update(['buy_vip_entry' => $entry, 'buy_last_date' => $lastDate]);
        return $entry;
    }
