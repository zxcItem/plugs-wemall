...
abstract class UserOrder
{
     ...
     * @throws ModelNotFoundException
     */
    public static function entry($unid): AccountRelation
    {
        [$relation, $unid] = AccountRelation::withRelation($unid);
        // 订单升级等级
        $query = ShopOrder::mk()->whereRaw('status>3 and refund_status<4');
    ...
    public static function perfect(ShopOrder $order, PaymentAddress $address): bool
    {
        ...
        // 记录原地址信息
        $data['extra'] = $data;
        ShopOrderSend::mk()->where(['order_no' => $orderNo])->findOrEmpty()->save($data);
        // 组装更新订单数据, 重新计算订单金额
        ...
        if ($order->save($update)) {
            // 触发订单确认事件
            Library::$sapp->event->trigger('ShopOrderPerfect', $order);
            // 返回处理成功数据
            return true;
        } else {
            return false;
